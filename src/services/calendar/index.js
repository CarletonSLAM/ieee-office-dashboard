...
export default { getData: () => new Promise(((resolve, reject) => {
    gapi.load('client', () => {
        gapi.client.init(config.google.client).then(() => gapi.client.calendar.events.list({ calendarId: config.google.calID,
            timeMin: (new Date()).toISOString(),
            showDeleted: false,
            singleEvents: true,
            maxResults: 15,
            orderBy: 'startTime' })).then(
            res => resolve(res.result),
            res => reject(res.result.error.message),
        )
    })
})),
transformResponse: response => response.items.map(({ summary, start, end, description, location }) => ({ summary,
