...
// Build server config
const bufferedConfig = Buffer.from(JSON.stringify(configFile, 'binary')).toString('base64')
...
if (!fs.existsSync(path.resolve(`${sslFolder}/server.cert`))) {
    console.log('SSL SELF-SIGNED CERTIFICATE NOT SET UP. CREATING ONE NOW..') // eslint-disable-line no-console
    spawn('openssl', [
        'req',
        '-nodes',
        '-new',
