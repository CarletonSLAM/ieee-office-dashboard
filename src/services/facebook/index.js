...
export default { getData: () => fetch(`https://graph.facebook.com/oauth/access_token?client_id=${facebook.client}&client_secret=${facebook.secret}&grant_type=client_credentials`)
...
transformResponse: responses => (responses[0].id ?
    responses.map(({ id, story, name, message, full_picture: src }) => ({ type: 'facebook', id, story, name, message, src })) :
