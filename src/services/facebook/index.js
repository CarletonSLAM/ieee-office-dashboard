...
export default {
    getAuth: () => fetch(`${AppConfig.server}/facebook`, { headers: generateHeaders() })
        .then(handleErrors)
    ...
    getData: () => new Promise((resolve, reject) => {
        ...
        .then(data => Promise.all(data.map(post => new Promise(resolve => FB.api(`/${post.id}`, { fields: postFields }, response => resolve(response)))))),
