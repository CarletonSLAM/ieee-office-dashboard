...
const aggregateTrips = (trips = []) => {
    ...
    return tripAggregate.map(trip => ({ dest: trip.TripDestination,
        time: moment(moment(Date.now()).add(trip.AdjustedScheduleTime, 'm')).format('LT') }))
}
export default { getData: () => Promise.all(Object.values(OCTranspoStops).map(stopNo => fetch(`http://localhost:8000/transpo?stopNo=${stopNo}`).then(
